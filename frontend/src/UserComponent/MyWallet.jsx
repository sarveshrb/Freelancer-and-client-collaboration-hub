...
const MyWallet = () => {
  ...
  const validateForm = (e) => {
    const errors = {};
    errors.walletAmount = validWalletAmount(e.walletAmount);
    return errors;
  };
  const validWalletAmount = (e) => {
    if (!e.trim()) {
      errors.walletAmount = "Amount is required";
    } else if (isNaN(e) || e <= 1000) {
      errors.walletAmount = "Amount must be greater than 1000";
    } else {
      errors.walletAmount = "";
    }
    return errors.walletAmount;
  };

  const addMoneyInWallet = (e) => {
    e.preventDefault();
    const validationErrors = validateForm(walletRequest);
            ...
            <form>
              <div className="mb-3 text-color">
                ...
                <input
                  ...
                  onInput={(e) => validWalletAmount(e.target.value)}
                  value={walletRequest.walletAmount}
                  required
                />
                {errors.walletAmount && (
                  <small className="text-danger">{errors.walletAmount}</small>
                )}
              </div>
